                ...
                if self.rrgame["System"]["Player Count"] < 6:
                    if self.enough_points(user, bet):
                        if not self.rrgame["System"]["Roulette Initial"]:
                            ...
                            await self.bot.say(user.name + " has started a game of Russian Roulette, with a starting bet of " +
                                               str(bet) + ".\n" "The game will start in 90 seconds or when I get 5 more participants."
                                               " Calling all " + discord.utils.get(ctx.message.server.roles, name="Risk Takers").mention)
